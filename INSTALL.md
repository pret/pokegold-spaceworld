...
These instructions explain how to set up the tools required to build **pokegold-spaceworld**, including [**rgbds**](https://github.com/gbdev/rgbds), which assembles the source files into a ROM.
...
WSL has its own file system that's not accessible from Windows, but Windows files *are* accessible from WSL. So you're going to want to install pokegold-spaceworld within Windows. You'll have to change the **current working directory** every time you open WSL.

For example, if you want to store pokegold-spaceworld in **C:\Users\\*\<user>*\Desktop**, enter this command:
...
Cygwin has its own file system that's within Windows, at **C:\cygwin64\home\\*\<user>***. If you don't want to store pokegold-spaceworld there, you'll have to change the **current working directory** every time you open Cygwin.

For example, if you want to store pokegold-spaceworld in **C:\Users\\*\<user>*\Desktop**:
...
Now you're ready to [build **pokegold-spaceworld**](#build-pokegold-spaceworld).
...
To install the software required for **pokegold-spaceworld**:
...
## Build pokegold-spaceworld

To download the **pokegold-spaceworld** source files:
...
If you have different projects that require different versions of `rgbds`, it might not be convenient to install rgbds 1.0.3 globally. Instead, you can put its files in a directory within pokegold-spaceworld, such as `pokegold-spaceworld/rgbds-1.0.3/`. Then specify it when you run `make`:
...
```bash
make RGBDS=rgbds-1.0.3/ gold_debug
```

```bash
make RGBDS=rgbds-1.0.3/ silver_debug
```
